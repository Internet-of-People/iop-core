...
UniValue dumpminerstats(const JSONRPCRequest& request) {
    ...
    if (request.fHelp || request.params.size() > 2) {
        throw std::runtime_error(
            ...
            "2. verbosity            (bool, optional) decides if all blocks generated by miner are shown\n"
            "\nResult if no address is given:\n"
            "{\n"
            ...
            "  \"stats\": \n"
            "  {\n"
            "    \"mineraddress\": n             (int) the number of blocks that this address mined in this window.\n"
            "  }\n"
            "}\n"
            "\nResult if an address is given:\n"
            "{\n"
            "  \"address\": address,             (str) the address you entered.\n"
            "  \"stats\": {\n                    only shown if address is known to the database\n"
            "  {\n"
            "    \"whitelisted\": n              (int) shows if the address is currently whitelisted.\n"
            "    \"windowBlocks\": n             (int) the number of blocks that this address mined in this window.\n"
            "    \"totalBlocks\": n              (int) the number of blocks that this address mined over all time.\n"
            "    \"lastBlock\": n                (int) the last block generated by this address.\n"
            "  }\n"
            "}\n"
        );
    }
    ...
    if (request.params.size() >= 1) {
        minerParameter = request.params[0].get_str();
        ...
        if (fVerbose) {
            ...
            if (pminerwhitelist->DumpFullStatsForMiner(minerParameter, &wlisted, &windowBlocks, &totalBlocks, blocks)) {
                ...
                UniValue lastBlocks(UniValue::VARR);
                for (unsigned int i = 0; i < blocks.size(); i++) {
                    lastBlocks.push_back((uint64_t)blocks[i]);
                }
                lastResult.push_back(Pair("lastBlocks",lastBlocks));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "blockchain",         "dumpminerstats",         &dumpminerstats,         true,  {"address","verbose"} },
