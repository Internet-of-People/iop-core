...
void IoPGUI::createToolBars()
{
    if (walletFrame) {
        ...
        toolbar->setIconSize(IoPStyle::getToolbarIconSize());
        ...
        progressBarLabel->setStyleSheet("background: transparent; color: " + IoPStyle::s_placeHolderText + "; margin-left: 30px; margin-right: 30px;");
...
void IoPGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to IoP network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void IoPGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void IoPGUI::updateHeadersSyncProgressLabel()
{
...
void IoPGUI::toggleNetworkActive()
{
    if (clientModel) {
